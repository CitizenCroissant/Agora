...
/** Inject path params into req.query so handlers find them */
...
export default async function handler(
  ...
  if (path === 'cron/notify-scrutins' && (method === 'GET' || method === 'POST')) {
  ...
  // Dynamic: sittings/:id
  if (!matched && segments[0] === 'sittings' && segments.length === 2 && method === 'GET') {
  ...
  // Dynamic: scrutins/:id
  if (!matched && segments[0] === 'scrutins' && segments.length === 2 && method === 'GET') {
    withParams(req, { id: segments[1] });
  ...
  if (!matched && segments[0] === 'deputy' && segments.length === 2 && method === 'GET') {
  ...
  if (!matched && segments[0] === 'deputies' && segments.length === 3 && segments[2] === 'votes' && method === 'GET') {
    withParams(req, { acteurRef: segments[1] });
  ...
  // Dynamic: groups/:slug
  if (!matched && segments[0] === 'groups' && segments.length === 2 && method === 'GET') {
  ...
  if (!matched && segments[0] === 'circonscriptions' && segments.length === 2 && segments[1] !== 'geojson' && method === 'GET') {
    withParams(req, { id: segments[1] });
