 ...
 */
const STATIC_DESCRIPTION_PAGES: { path: string; changeFrequency: 'yearly' | 'monthly' | 'weekly' | 'daily'; priority: number }[] = [
...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  return STATIC_DESCRIPTION_PAGES.map(({ path, changeFrequency, priority }) => ({
    url: `${baseUrl}${path}`,
